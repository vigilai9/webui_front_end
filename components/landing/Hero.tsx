...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    if (!isDeleting && visibleLetters < words[currentWordIndex].length) {
      ...
      !isDeleting &&
      visibleLetters === words[currentWordIndex].length
    ) {
      timeout = setTimeout(() => {
        setIsDeleting(true)
      }, 500)
        ...
        <div className='flex flex-col lg:flex-row justify-between gap-8 w-full py-2'>
          <div className='flex-1 space-y-6 text-left w-full'>
            <h1 className='text-4xl sm:text-5xl md:text-4xl font-bold text-gray-900 leading-tight'>
              Intelligent <br />
              <span className='text-[#1b3b5f] text-3xl sm:text-4xl md:text-4xl'>
                {visibleText}
              </span>
            ...
            <p className='text-gray-500 text-sm max-w-xl'>
              Detect incidents in under 30 seconds with GenAI powered video
              surveillance. Receive instant alerts and comprehensive analytics
              for total security awareness.
            </p>
            ...
            <div className='flex flex-wrap gap-4 pt-4'>
              <Button
                ...
                variant={'outline'}
                asChild
                className='rounded bg-[#1b3b5f] hover:bg-[#1b3b5f] hover:text-white hover:translate-y-0.5 transition-all duration-300 text-white py-4'
              >
              ...
              <Button
                ...
                variant={'outline'}
                asChild
                className='rounded bg-white hover:translate-y-0.5 transition-all duration-300 py-4'
              >
          ...
          <div className='rounded-lg overflow-hidden w-full sm:w-[80%] md:w-[60%] lg:w-[40%] max-w-xl mx-auto'>
            ...
            <div className='flex justify-between items-center py-2'>
              <div className='flex justify-center w-full py-1 px-2'>
                <div className='shadow-lg px-2 py-1 border border-gray-200 rounded'>
                  Without VigilAI
                </div>
              </div>
              <div className='flex justify-center w-full py-1 px-2'>
